...
def main():
    # 1. Read CSV
    input_csv = "athena_spmv_cgSpmv_cgAll_baseline_median.csv"  # replace with actual CSV filename
    ...
    # --- NEW STEP: Filter rows by the ratio condition median_GFLOPS_yax_basic / median_GFLOPS_yax_xyp_basic > 1.2 ---
    # Assuming that the data does not contain zero or negative values in the denominator: Dropping such cases now.
    df = df[
        df["median_GFLOPS_yax_xyp_basic"] != 0  # prevent divide-by-zero
    ]
    df = df[
        (df["median_GFLOPS_yax_basic"] / df["median_GFLOPS_yax_xyp_basic"]) > 1.2
    ]
    ...
    output_csv = "output_correlations_better_yax.csv"
